...
// Answer: When we use one equal sign we assign the value on the right to the value on the left and when 2 equal signs are used, we compare the left and the right term and the return is true if the condition is satisfied(they are equal) and false otherwise.
...
func countToOneHundred(count: Int) -> Int {
    ...
    if count == 100 {
        ...
        print("Number is too high, start counting from 1, please.")
        return 0
    }
}
